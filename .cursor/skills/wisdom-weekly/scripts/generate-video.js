...
function parseArgs(argv) {
  ...
  return { positional, seconds, withAudio };
}
...
function muxAudioVideo(videoPath, bgmPath, voicePath, outputPath) {
  ...
  const filter = [
    `[1:a]atrim=0:${dur},asetpts=PTS-STARTPTS,volume=${BGM_VOLUME}[bgm]`,
    `[2:a]aresample=44100,aformat=channel_layouts=mono,adelay=${delay}|${delay},volume=${VOICE_VOLUME},afade=t=in:st=0:d=0.5[voice]`,
    `[bgm][voice]amix=inputs=2:duration=longest:dropout_transition=2,apad=whole_dur=${dur}[aout]`,
  ].join(";");
...
function main() {
  const { positional, seconds, withAudio } = parseArgs(process.argv);
  ...
  if (withAudio) {
    ...
    if (!synthesizeSpeech(narration, voicePath)) {
      ...
      const bgmPath = ensureDefaultBgm();
      console.log("正在混音 BGM + 旁白…");
      muxAudioVideo(silentPath, bgmPath, voicePath, outputPath);
      try {
  ...
  if (fs.existsSync(wechatPath)) {
    const h1 = fs.readFileSync(wechatPath, "utf8").match(/^#\s+(.+)/m);
    if (h1) wechatTitle = h1[1].trim();
  }
  writeChannelsPost(weekDir, { title: xhsMeta.title || wechatTitle }, withAudio);
